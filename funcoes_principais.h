...
void marcar_missao(Total_missoes *total_missoes,
  ...
  // criação da equipa
  criar_equipa(todos_militares, todas_funcoes,
               &tipos_missao->conj_tipos_missao[escolha], data_final,
               total_missoes, escolha);
}
...
void listar_missoes(Total_missoes *lista_de_missoes,
  ...
  for (int i = 0; i < lista_de_missoes->cont_missoes; i++) {
    ...
    if (data >= data1 && data <= data2) {
      ...
      printf("\n%d\n%d//%d//%d\n%s", lista_de_missoes->conj_missoes[i].n_voo,
             data / 10000, data % 10000 / 100, data / 100,
             lista_tipos_missao
  ...
  printf("deseja ver detalhes de uma missao, se desejar introduza o numero de "
         "voo ou introduza -1 para sair");
  scanf("%d", &escolha);
}

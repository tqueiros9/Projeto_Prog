...
typedef struct mission_type { // a estrutura mission_type tem 3 variáveis char,
                              // int, int
...
typedef struct tripulante {
  ...
  int funcao; // funcao de cada tripulante, 1 piloto cmdt, 2 piloto, 3 nav, 4
              // load master,5 recuperador, 6 piloto instrutor, 7 aluno
...
typedef struct voo {
  ...
  Tripulante conj_trip[MAX_TIPOS]; // vetor do tipo "tripulantes" com nome
                                   // "conjunto detripulantes" [] tamanho
} Missao_unica;
...
typedef struct total_tipos_missao {
  Mission_type conj_tipos_missao[MAX_TIPOS]; //[mission_type1] [mission_type2]
                                             //[mission_type3]....
